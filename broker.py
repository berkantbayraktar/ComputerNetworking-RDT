        ...
        if payload: 
           ...
           # print('rand:',rand)
            if rand == 1 : 
                ...
                for i in range(WINDOW_SIZE):
                    udp_socket_r1.sendto(str(checksum_lenght[i]) + checksum_string[i] + payload[i],(destination_ip_1,udp1_port))
                    print(payload[i])
            ...
            # otherwise send to destination via router_2
                ...
                for i in range(WINDOW_SIZE):
                    udp_socket_r2.sendto(str(checksum_lenght[i]) + checksum_string[i] +payload[i],(destination_ip_2,udp2_port))
                    print(payload[i])
